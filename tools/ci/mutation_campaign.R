...
CATALOGUE <- list(
  ...
  list(id = "nickname-prefix-heuristic-smuggled",
       file = "R/nicknames.R",
       find = "    if (length(intersect(cx, cy))) \"corroborates\" else \"conflicts\"",
       repl = "    if (length(intersect(cx, cy)) || substr(x, 1, 3) == substr(y, 1, 3)) \"corroborates\" else \"conflicts\"",
       why  = "a three-character prefix becomes nickname evidence; ELISABETH corroborates ELIZABETH and the mystery_shopper defect goes canonical"),
