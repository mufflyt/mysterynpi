...
CATALOGUE <- list(
  ...
  list(id = "npi-search-policy-gate-removed",
       file = "R/npi_search.R",
       find = "  if (identical(name_expansion, \"curated_one_hop\") &&\n      identical(source_class, \"formal_record\")) {",
       repl = "  if (FALSE) {",
       why  = "formal legal-name rosters silently regain nickname expansion the ablation showed is all cost"),
  list(id = "npi-search-policy-gate-removed-2",
       file = "R/nickname_policy.R",
       find = "  bad <- which(accepted$review_only %in% TRUE)",
       repl = "  bad <- integer(0); which(accepted$review_only %in% TRUE)",
