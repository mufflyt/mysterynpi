...
CATALOGUE <- list(
  ...
  list(id = "nickname-formal-weld",
       file = "R/nicknames.R",
       find = "formals_of <- split(edges$name, edges$nickname)",
       repl = "formals_of <- split(c(edges$name, edges$nickname), c(edges$nickname, edges$name))",
       why  = "formal names sharing a hub nickname become equivalent through the nickname side"),
  ...
  list(id = "nickname-initial-flip",
       file = "R/nicknames.R",
       find = "if (nchar(x) == 1L || nchar(y) == 1L) {\n      return(\"uninformative\")\n    }",
       repl = "if (nchar(x) == 1L || nchar(y) == 1L) {\n      return(\"corroborates\")\n    }",
       why  = "a single-letter initial is restored as nickname evidence"),
  list(id = "nickname-absence-flip",
       file = "R/nicknames.R",
       find = "if (!has_name_information(x) || !has_name_information(y)) {\n      return(\"uninformative\")\n    }",
       repl = "if (!has_name_information(x) || !has_name_information(y)) {\n      if (is.na(x) && is.na(y)) return(\"corroborates\") else return(\"uninformative\")\n    }",
       why  = "shared missing first names become nickname corroboration"),
  list(id = "nickname-prefix-flip",
       file = "R/nicknames.R",
       find = "if (x == y) return(\"corroborates\")",
       repl = "if (x == y || substr(x, 1L, 3L) == substr(y, 1L, 3L)) return(\"corroborates\")",
       why  = "three-character prefixes are smuggled into nickname evidence"),
  list(id = "status-expired-becomes-retired",
       file = "R/license_status.R",
       find = "retired = c(\"RETIRED\", \"VOLUNTARILY RETIRED\",",
       repl = "retired = c(\"RETIRED\", \"EXPIRED\", \"VOLUNTARILY RETIRED\",",
       why  = "a lapse counts as retirement; the 4x inflation, one list entry away"),
  list(id = "status-deceased-unmapped",
       file = "R/license_status.R",
  ...
  list(id = "npi-search-alias-back-on",
       ...
       repl = 'if (FALSE) params <- c(params, use_first_name_alias = "False")',
       why  = "NPPES resumes alias-expanding queries against a list nobody can audit"),
  list(id = "npi-search-fanout-unguarded",
       file = "R/npi_search.R",
       find = "if (nrow(out) > max_expansion) {",
       repl = "if (FALSE) {",
       why  = "a dictionary defect explodes one query into unbounded fan-out silently"),
  list(id = "npi-search-dedup-dropped",
       file = "R/npi_search.R",
       find = "providers <- providers[!duplicated(providers$npi), , drop = FALSE]",
       repl = "providers <- providers",
       why  = "expanded searches return one row per (provider, variant); counts inflate by fan-out"),
  list(id = "nickname-variants-transitive",
       file = "R/npi_search.R",
       find = "  v <- sort(setdiff(unique(c(edges$nickname[edges$name == k],\n                             edges$name[edges$nickname == k])), k))",
       repl = "  v <- sort(setdiff(unique(c(edges$nickname[edges$name == k],\n                             edges$name[edges$nickname == k])), k))\n  for (q2 in v) v <- unique(c(v, edges$nickname[edges$name == q2],\n                              edges$name[edges$nickname == q2]))\n  v <- sort(setdiff(v, k))",
       why  = "expansion walks the graph: BILL reaches FREDERICK through FRED, hub welds return"),
  list(id = "npi-search-provenance-corrupted",
       file = "R/npi_search.R",
       find = "    for (col in names(plan)) p[[col]] <- rep(plan[[col]][i], nrow(p))",
       repl = "    for (col in names(plan)) p[[col]] <- rep(plan[[col]][1], nrow(p))",
       why  = "every returned row claims the identity path; which alias found a provider is unknowable"),
  list(id = "nickname-variant-edge-id-dropped",
       file = "R/npi_search.R",
       find = "                    alias_edge_id = c(NA_character_, edge_ids),",
       repl = "                    alias_edge_id = rep(NA_character_, length(c(k, v))),",
       why  = "nickname expansion paths lose the exact edge that licensed each query"),
  list(id = "nickname-variant-version-dropped",
       file = "R/npi_search.R",
       find = "                    alias_dictionary_version = ver,",
       repl = "                    alias_dictionary_version = NA_character_,",
       why  = "nickname expansion paths lose the dictionary version they came from"),
  list(id = "npi-search-lineage-discarded",
       file = "R/npi_search.R",
       find = "    providers$found_by_edges <- as.character(tapply(path, f, joinu))",
       repl = "    providers$found_by_edges <- providers$alias_edge_id",
       why  = "dedup silently discards the other paths that found a provider; lineage unreconstructable"),
  list(id = "npi-search-policy-gate-removed",
       file = "R/npi_search.R",
       find = "    if (!identical(perm, \"review_only\")) {",
       repl = "    if (FALSE) {",
       why  = "the registry verdict is ignored: formal and unknown sources silently regain expansion"),
  list(id = "npi-search-policy-gate-removed-2",
       file = "R/nickname_policy.R",
       find = "  bad <- which(accepted$review_only %in% TRUE |\n                 accepted$acceptance_contribution %in% \"nickname_only\")",
       repl = "  bad <- integer(0)",
       why  = "the auto-accept guard waves every nickname-only candidate through"),
  list(id = "unknown-source-class-informal",
       file = "R/nickname_policy.R",
       find = "  expansion = c(\"forbidden\", \"review_only\", \"fail_closed\"),",
       repl = "  expansion = c(\"forbidden\", \"review_only\", \"review_only\"),",
       why  = "an unclassified source silently earns expansion; fail-closed becomes fail-open"),
  list(id = "npi-search-lineage-column-dropped",
       file = "R/npi_search.R",
       find = "  providers$source_class <- rep(source_class, nrow(providers))",
       repl = "  invisible(rep(source_class, nrow(providers)))",
       why  = "candidates stop carrying their source class; the lineage contract quietly thins"),
  list(id = "nickname-only-not-review-only",
       file = "R/npi_search.R",
       find = "  providers$review_only <- !is.na(providers$found_by_edges) &\n    !grepl(\"(^|\\\\|)input($|\\\\|)\", providers$found_by_edges)",
       repl = "  providers$review_only <- rep(FALSE, nrow(providers))",
       why  = "nickname-only candidates masquerade as direct evidence and sail past the guard"),
  list(id = "ablation-fixture-altered",
       file = "tests/testthat/fixtures/ablation/ablation_pins_v2.csv",
       find = "with_table,auto_accept_true,115",
       repl = "with_table,auto_accept_true,116",
       why  = "the governing evidence is edited in place without a policy supersession"),
  list(id = "npi-fetch-reads-dictionary",
       file = "R/npi_search.R",
       find = "npi_fetch_impl <- function(u) {\n  con <- url(u)",
       repl = "npi_fetch_impl <- function(u) {\n  invisible(mysterynpi::NICKNAME_EDGES$nickname)\n  con <- url(u)",
       why  = "a second dictionary reader appears outside the canonical module; the invariant must catch it"),
  list(id = "drift-scanner-local-map-blind",
       file = "tools/audit/check_external_nickname_drift.R",
       find = "\\\\bNICKNAMES?(_MAP)?\\\\s*<-",
       repl = "\\\\bNO_SUCH_NICKNAME_MAP_PATTERN\\\\s*<-",
       why  = "owner-wide scanner stops detecting a reintroduced Strategy 19 local nickname map"),
  list(id = "drift-scanner-alias-true-blind",
       file = "tools/audit/check_external_nickname_drift.R",
       find = "use_first_name_alias\\\\s*=\\\\s*(TRUE|True|true|['\\\"]TRUE['\\\"]|['\\\"]True['\\\"]|['\\\"]true['\\\"])",
       repl = "NO_SUCH_ALIAS_TRUE_PATTERN",
       why  = "owner-wide scanner stops detecting NPPES aliasing switched back on"),
  list(id = "drift-scanner-midwifery-shim-blind",
       file = "tools/audit/check_external_nickname_drift.R",
       find = "source\\\\s*\\\\([^\\\\n]*nickname_system[.]R",
       repl = "NO_SUCH_MIDWIFERY_SHIM_PATTERN",
       why  = "midwifery can again route nickname semantics through the isochrones shim"),
  list(id = "drift-scanner-prefix-nickname-blind",
       file = "tools/audit/check_external_nickname_drift.R",
       find = "(nickname(.|\\\\n){0,160}substr\\\\s*\\\\([^\\\\n]*,\\\\s*1\\\\s*,\\\\s*3\\\\s*\\\\)|substr\\\\s*\\\\([^\\\\n]*,\\\\s*1\\\\s*,\\\\s*3\\\\s*\\\\)(.|\\\\n){0,160}nickname)",
       repl = "NO_SUCH_PREFIX_AS_NICKNAME_PATTERN",
       why  = "owner-wide scanner stops detecting source-prefix heuristics labelled as nickname evidence"),
